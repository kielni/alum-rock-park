...
slope_class = Layer(
    ...
    processing_step=ProcessingStep(
        description=(
            "Reclassify continuous slope degrees into five integer categories"
            " (1=flat, 2=gentle, 3=moderate, 4=steep, 5=very steep) based on mechanical"
            " treatment access breakpoints."
        ),
        algorithm=(
            "gdal_calc.py -A {input} --A_band=1 -B {input} --B_band=2"
            ' --outfile={output} --calc="numpy.where(B>0,'
            ' numpy.select([A<5, A<11, A<22, A<31], [1,2,3,4], 5), 0)"'
